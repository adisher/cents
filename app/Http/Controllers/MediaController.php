...
class MediaController extends Controller
{
    ...
    private function getUserFriendlyError(\Exception $e, $fileName): string
    {
        ...
        if (str_contains($message, 'storage') || str_contains($message, 'disk')) {
            return __('Storage error: :extension', ['extension' => $message]);
        }
        ...
        if (str_contains($message, 'permission') || str_contains($message, 'denied')) {
            return __('Permission denied: :extension', ['extension' => $message]);
        }
        ...
        foreach ($request->file('files') as $file) {
            try {
                ...
                \Log::info('Media uploaded to S3', [
                    ...
                    'file_name'     => $media->file_name,
                    'disk'          => $media->disk,
                    'path'          => $media->getPath(),                                    // Full path on disk
                    'relative_path' => $media->getPathRelativeToRoot(),                      // Path relative to bucket root
                    'bucket_path'   => $media->disk . '/' . $media->getPathRelativeToRoot(), // Full S3 path
                    'url'           => $media->getUrl(),
