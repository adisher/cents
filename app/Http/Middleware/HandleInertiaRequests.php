...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        if ($request->is('install/*') || $request->is('update/*') || ! file_exists(storage_path('installed'))) {
            ...
            //     $settings = settings();
            // }
            // Get system settings (workspace-specific for company users and their workspace members)
            $user = $request->user();
            if ($user && $user->current_workspace_id) {
                // For non-company/non-superadmin users, get settings from workspace owner
                if (! in_array($user->type, ['superadmin', 'company'])) {
                    $workspace = $user->currentWorkspace;
                    $ownerId   = $workspace ? $workspace->owner_id : null;
                    $settings  = $ownerId ? settings($ownerId, $user->current_workspace_id) : settings();
                }
                // For company users in SaaS mode, get their own workspace settings
                elseif ($user->type === 'company' && isSaasMode()) {
                    $settings = settings($user->id, $user->current_workspace_id);
                } else {
                    $settings = settings();
                }
